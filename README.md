...
## Experiments 
 While implementing our project we created adjacent matrices and counted for a number of vertices and clusters. We also calculated the cost of each such change.
 We answered one of the important questions of how does the running time depend on the number of vertices of the input graph, the answer is,  for 10 vertices, we need to test $ 2^((((vertexcount - 1)^2)(vertexcount-1)) / 2)$ computations. For normal computers, it would be too time-consuming to calculate. 
